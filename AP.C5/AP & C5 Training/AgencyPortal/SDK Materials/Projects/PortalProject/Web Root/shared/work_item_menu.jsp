		...
		<div class="row">
			<div id="wi-reference"
				...
				<% if (jspHelper.showProgressBar()) {
						...
						%>
				<div class="progress pull-left" style="width: 20%">
				...
				<span id="lob-title" class="lob-title <%= transaction.getLob() %>">
					<%= transaction.getTitle() %> (ID: <%= JSPHelper.prepareForHTML(workItemTitle) %>)
				</span>
			</div>

			<div id="wi-options" class="hidden-xs col-sm-6 text-right">

				<% if(null != accountUrl){ %>
				<span id="accountReference"><a href="<%= accountUrl %>">
				...
				<% } %>
				<span id="ap_autosave_list" <% if(lastAutoSaved == null) { %>
					style="display: none" <% } %>>
					<p><%=rb.getString("title.autosave") + ":"%>
