	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h2 class="modal-title">${ACCOUNT_MANAGEMENT_RB['account.actions.merge.account']} </h2>
			</div>
			<div class="modal-body">
				<div id="merge-account_container" >
					<p id="instruction_tip">${ACCOUNT_MANAGEMENT_RB['account.actions.merge.account.searchAcctMoveTip1']}
						<b>
							&ldquo;
							<%=JSPHelper.prepareForHTML(details.getAccountName())%>
							&ndash; ${ACCOUNT_MANAGEMENT_RB['account.management.label.accountHashTag']}  <%=details.getAccountNumber()%>
							&rdquo;
						</b>
					</p> 
					<div class="row">
						<div class="col-lg-12">
							<div class="input-group">
								<input type="text" class="form-control" placeholder=" ${ACCOUNT_MANAGEMENT_RB['account.actions.merge.account.searchbox']}" name="autocomplete_parameter" id="merge-account-search-input" >
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<p id="merge-account-confirm-message">
					${ACCOUNT_MANAGEMENT_RB['account.actions.merge.account.confirm']}
					<b>
						&ldquo;
							<%=JSPHelper.prepareForHTML(details.getAccountName()) %>
						&rdquo;
				...
				<div class="button-row">
					<button type="button" class="btn btn-primary" data-dismiss="modal">${ACCOUNT_MANAGEMENT_RB['modal.confirmation.button.yes']}</button>
					<button type="button" class="btn btn-default" data-dismiss="modal">${ACCOUNT_MANAGEMENT_RB['modal.confirmation.button.no']}</button>
				</div>
			</div>
		</div>
	</div>
